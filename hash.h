...
public:
  ...
  { return ((m_newTable == TABLE1) ? m_capacity1 : m_capacity2); }
  // Returns number of entries in the table
  unsigned int numEntries(TABLENAME tableName) const 
  { return ((m_newTable == TABLE1) ? m_size1 : m_size2); }
  // Returns Load factor
  ...
  File getFile(string name, int diskBlock);
  void dump() const;
private:
  ...
  bool isRehashing; //keeps tack of when rehashing happens
  double resizeInc; //progress of rehash
  void rehash(double inc); //rehash function
};
#endif
